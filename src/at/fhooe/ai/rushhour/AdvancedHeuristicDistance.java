...
public class AdvancedHeuristicDistance implements Heuristic {
			...
			if (i != car && !blockingCars.contains(i)) {
				...
				if (this.isCarRelevent(car, i, state, true)) {
					if (!this.isEnoughSpace(car, i, moveSpaceFront, true, state)) {
						movesForward += this.getAllBlockingCars(i,
								this.getMoveSpace(car, i, moveSpaceFront, state, true),
				...
				if (this.isCarRelevent(car, i, state, false)) {
					if (!this.isEnoughSpace(car, i, moveSpaceBack, false, state)) {
						movesBackward += this.getAllBlockingCars(i,
								this.getMoveSpace(car, i, moveSpaceFront, state, true),
								this.getMoveSpace(car, i, moveSpaceBack, state, false), state);
					}
	...
	private boolean isEnoughSpace(int car, int blockingCar, int spaceNeeded, boolean forward, State state) {
		...
		if (forward) {
			if (this.isOrientationEqual(car, blockingCar)) {
				return spaceNeeded <= Math.abs(state.getVariablePosition(car) + this.puzzle.getCarSize(car)
						- state.getVariablePosition(blockingCar));
			}
			return spaceNeeded <= Math.abs(state.getVariablePosition(car) + this.puzzle.getCarSize(car)
					- this.puzzle.getFixedPosition(blockingCar));
		} else {
			if (this.isOrientationEqual(car, blockingCar)) {
				return spaceNeeded <= Math.abs(state.getVariablePosition(car)
						- (state.getVariablePosition(blockingCar) + this.puzzle.getCarSize(blockingCar)));
			}
			return spaceNeeded <= Math
					.abs(state.getVariablePosition(car) - (this.puzzle.getFixedPosition(blockingCar) + 1));
		}
	...
	private boolean isCarRelevent(int car, int blockingCar, State state, boolean forward) {
		if (this.isOrientationEqual(car, blockingCar)) {
			return (state.getVariablePosition(blockingCar) < state.getVariablePosition(car) && !forward)
					|| (state.getVariablePosition(blockingCar) > state.getVariablePosition(car) && forward);
		}
