...
public class Puzzle {
  ...
  /**
   * Returns the orientation of car <tt>v</tt>, where <tt>true</tt> means that the
  ...
  /**
   * Returns the current value of the search counter, which keeps a count of the
   * number of nodes generated on the current search.
   */
  ...
  /**
   * The main constructor for constructing a puzzle. You probably will never need
   * to use this constructor directly, since ordinarily puzzles will be
  ...
  public Puzzle(String name, int gridSize, int numCars, boolean orient[], int size[], int x[], int y[]) {
    ...
    for (int v = 0; v < numCars; v++) {
      ...
      if (x[v] < 0 || y[v] < 0 || (orient[v] && y[v] + size[v] > gridSize) || (!orient[v] && x[v] + size[v] > gridSize))
  ...
  /**
   * A static method for reading in a list of puzzles from the data file called
   * <tt>filename</tt>. Each puzzle is described in the data file using the format
   * described on the assignment. The set of puzzles is returned as an array of
   * <tt>Puzzle</tt>'s.
   */
  public static Puzzle[] readPuzzlesFromFile(String filename) throws FileNotFoundException, IOException {
    ...
    try (BufferedReader in = new BufferedReader(new FileReader(filename))) {
      ...
      return puzzles.toArray(new Puzzle[0]);
    }
  }
